...
// 根据交易ID获得
BlockInfo info = channel.queryBlockByTransactionID(txid);
// 根据Hash获得
BlockInfo info = channel.queryBlockByHash(hash);// 根据Hash获得
// 获取Hash
Hex.encodeHexString(returnedBlock.getDataHash())；// 数据的Hash
Hex.encodeHexString(returnedBlock.getPreviousHash())； // 前一个区块的Hash
Hex.encodeHexString(SDKUtils.calculateBlockHash(client, // 计算出当前区块的Hash
...
```java
//根据交易ID获取教习
TransactionInfo info1 = channel.queryTransactionByID(txID);

// 获取当前信封的交易ID
int envelopCount = envelopeInfo.getTransactionID();
// 获取当前信封所在的通道
